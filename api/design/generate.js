/**
 * Vercel Serverless Function - 阿里通义图像编辑 API 代理
 * 
...
export default async function handler(req, res) {
  ...
  try {
    const { background_image, foreground_image, prompt, product_type } = req.body;
...
function getPromptByProductType(productType) {
  const prompts = {
    ...
    pergola: '请将第二张图中的凉亭自然地放置到第一张后院实景照片中。确保凉亭与周围环境协调，阴影和光线效果真实自然，比例和透视正确。',
