...
│ · 产品目录浏览 │  │ · 选择产品后:                   │
│ · 材料成本价   │  │   "查看材料成本" 按钮           │
│   (BOM Cost)  │←─│   → 跳转 Products 页           │
│ · 创建/编辑    │  │     查看详细规格和 BOM 明细      │
│   产品        │  │                               │
│ · 管理配置选项 │  │ · 叠加计算:                     │
│ · 文件管理    │  │   人工 + 设备 + 许可 + 利润      │
│              │  │ = 最终报价                      │
...
**跳转方向：Pricing Controller → Products（查看材料成本明细）**

报价人员在 Pricing Controller 中选择产品后，可通过"查看材料成本"按钮跳转到 Products 页面，
查看该产品的完整规格、组件清单和各跨度的材料成本基准价，然后返回继续完成报价计算。
...
**跳转方向：Pricing Controller → Products**

- Pricing Controller 选择产品后，在产品信息区域新增 **"查看材料成本"** 按钮
- 点击后自动跳转到 Products 页面，并选中对应产品
- Products 页面展示该产品的完整规格、组件清单、各跨度的材料成本基准价
- 报价人员查阅后可通过侧边栏导航返回 Pricing Controller 继续报价

### 2.6 Products 页产品管理能力增强（后续规划）

Products 页将从"只读产品目录"升级为"产品管理中心"，包括：

| 能力 | 说明 | 优先级 |
|------|------|--------|
| **创建产品** | 填写产品名称、类别、系列、组件、跨度等信息，新建 SKU | P1 |
| **编辑产品** | 修改现有产品的规格、配置选项、材料成本价 | P1 |
| **管理材料成本** | 按跨度区间设置/更新各产品的材料成本基准价 | P1 |
| **管理配置选项** | 新增/编辑/删除产品类别的可选配置项 | P2 |
| **产品状态管理** | Active / Draft / Archived 状态切换 | P2 |
| **文件管理** | 上传产品图纸、安装手册等附件 | P2 |

### 2.7 Pricing Controller 数据源升级
...
| 优先级 | 改动 | 复杂度 |
|--------|------|--------|
| **P0** | Pricing Controller 加 "查看材料成本" 跳转按钮（→ Products 页） | 低 |
| **P1** | Pricing Controller 产品选择器改为读 productCatalog | 中 |
| **P1** | 配置选项从 productOptionSets 动态加载 | 中 |
| **P1** | Products 页增强 Create / Edit Product 能力 | 中 |
| **P1** | Products 页增加材料成本管理功能 | 中 |
