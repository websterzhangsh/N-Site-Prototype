...
| **Step 3** | 激光三维测量，方案精准呈现 | 用精确数据锁定设计，消除安装偏差 | — |
...
graph TB
    ...
    S2 --> S3["Step 3<br/>激光三维测量<br/>方案精准呈现"]
...
> *注：Step 2 为概念阶段，重在"快速可视化激发渴望"。完整的设计赋能全矩阵（7 种产出）在 Step 3 激光三维测量后交付。*
...
| 2 | **激光三维扫描** | 使用 UNRE 激光扫描仪（若 UNRE 无法进入美国市场，则 Leica 作为备选），进行毫米级精度扫描。扫描范围根据客户需求选择：**基础版**（安装区域及周边必要结构：墙体、地面、屋檐）/ **增值版**（全场景扫描，为客户提供完整点云模型数字资产，可额外收费） |
| 3 | **结构评估** | 基于点云数据分析墙体承重、地基状况、与主屋连接点的结构可行性，出具初步评估建议 |
| 4 | **障碍物精确定位** | 记录所有管道、烟道、通风口、水电接口、地下设施（化粪池、排水管）的精确位置（结合客户提供的产权图或现场探勘） |
| 5 | **合规性初筛** | 基于精确位置复核退界距离、排水坡度、与邻居边界距离等是否符合当地法规 |
| 6 | **深化设计** | 基于三维点云模型，将客户选定的标准产品（与产品矩阵对应）/半标产品/非标产品模型嵌入场景，运用 AI 设计工具生成任意角度的实景融合方案效果图（Photorealistic Schematic Design）+ 总平面规划图（Site Plan），标注阳光房位置、尺寸、与周边关系。*（自动出图方案形成前，先用手动出图）* |
| 7 | **方案呈现** | 向客户展示精准方案，确认所有细节（外观、内部布局、与现有建筑的衔接方式），**客户签字确认方案** |

### 4.2.1 测量能力演进（对齐白皮书 4.2.1）

<mark>**激光三维扫描为主要测量方式**</mark>（UNRE 首选，Leica 备选），替代传统两次上门测量：

| 阶段 | 测量方式 | 精度 | 上门次数 | 状态 |
|------|---------|------|---------|------|
| **当前** | 激光三维扫描（UNRE/Leica） | 毫米级 | 1 次扫描 | **实施中** |
| **增强** | AI + 激光融合，一次上门完成初测+复核+自动校验 | 毫米级 + 自动校验 | **1 次** | 规划中 |
...
### B.3 第三阶段：激光三维测量与方案深化

| 任务项 | 责任方 | 输入/要求 | 输出/成果 | 完成确认 |
|--------|--------|---------|---------|---------|
| 预约现场测量时间 | 客户 | — | 确认测量日期 | □ |
| 激光三维扫描 | 测量师 | 现场 | 毫米级点云数据 | □ |
| 结构评估与合规初筛 | 设计师 | 点云数据 + 场地信息 | 结构评估报告、合规指引 | □ |
...
| **3.0** | **2026-03-16** | **整合三份新文档**：激光三维扫描升级为 Step 3 主要测量方式（UNRE 首选/Leica 备选）；Step 1 产出引用附件 A（8 模块完整问卷）；Step 4 报价明细扩充（运费含关税、基础建设费用提示）；Step 5 新增远程验收；新增附件 A（Client Design Intake Questionnaire）；新增附件 B（Project Management Checklist 全 6 阶段含签字确认）；三维点云模型列为设计矩阵产出 |
